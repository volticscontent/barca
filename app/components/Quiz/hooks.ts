...
export const useTikTokClickIdCapture = () => {
  useEffect(() => {
    ...
    if (ttclid) {
      ...
      // Set cookie as well
      document.cookie = `ttclid=${ttclid}; path=/; max-age=2592000; SameSite=Lax`; // 30 days
    }
  }, []);
};
...
export const useQuizLogic = () => {
  ...
  const handleBuyNowClick = useCallback((selectedKit: string, options?: unknown) => {
    trackQuizStep('go_to_store');
    
    // Log options for debugging or future integration
    if (options) {
